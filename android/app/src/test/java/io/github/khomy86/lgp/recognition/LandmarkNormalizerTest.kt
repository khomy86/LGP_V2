...
class LandmarkNormalizerTest {
    ...
    // Same fixture as ml/tests/fixtures/normalization.json
    ...
    fun matchesPythonImplementation() {
        assertArrayEquals(expected, LandmarkNormalizer.normalize(hand, 480, 640), 1e-5f)
    }
    ...
    fun isInvariantToHandPositionAndSize() {
        ...
        assertArrayEquals(
            LandmarkNormalizer.normalize(hand, 480, 640),
            LandmarkNormalizer.normalize(moved, 480, 640),
            1e-5f
        )
    }
    ...
    fun farthestPointFromWristHasUnitDistance() {
        val points = LandmarkNormalizer.normalize(hand, 480, 640)
